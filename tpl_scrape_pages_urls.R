...
while (urlpdfloop < plist + 1) {
  ...
  try (dataFile <- remDr$findElements(using = "xpath", ".//*[@class='listingItemLI detailListingItem']"))
  try (title_temp <- remDr$findElements(using = "xpath", ".//*[@class='itemTitle']"))
  try (language_temp <- remDr$findElement(using = "xpath", ".//*[@class='file-details btn-default trsp-btn active']"))
  try (closeButton <- remDr$findElement(using = "xpath", ".//*[@class='modalClose']"))
  ...
  #we collect the metadata of the files
  ...
  #Get the active language of the document
